# Sentiment & Priority Labeling Prompt — General Banking Support Ticket Labeler
...
**A. Does the message itself carry real emotional distress?** Not "something bad happened" — the *writing* itself reads as angry, frustrated, or panicked, in whatever words the customer chose. A customer venting, escalating, or describing being worn down by repeated failed attempts counts here, regardless of the specific vocabulary used. A customer calmly stating a fact does not, no matter how bad the fact is.

**B. Does the customer say something was done TO them without their consent, framed as a violation — a specific denial ("I didn't do that") or a concrete claim that money has vanished with no explanation?** This is about the *shape* of the claim (personal accusation / unexplained loss), not the topic. It has to be the customer's own claim of being wronged, not just a topic that sounds serious.

Label **Negative** if either A or B is true. Otherwise **Neutral**.
...
When a ticket sits ambiguously between "calm report" and "implied grievance," default **Neutral**. A false Negative misdirects triage urgency more than a missed one does, and even careful human labelers disagree at this margin — that disagreement is real, not a gap in the rule.
