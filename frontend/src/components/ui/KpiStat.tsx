...
export function KpiStat({
  ...
  return (
    <div
      className={cn(
        "rounded-lg border border-border-subtle bg-surface-card p-5 shadow-xs",
        className,
      )}
    >
      ...
      <div className="mb-3 text-[30px] font-bold leading-tight text-text-primary">
        {value}
      </div>
      {delta ? (
        <div className="flex items-center gap-1.5 border-t border-border-subtle pt-3">
          <span
            className={cn(
              ...
              deltaTone === "success" ? "text-success-text" : "text-error-text",
            )}
          >
            {delta}
          </span>
