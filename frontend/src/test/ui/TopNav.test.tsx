...
describe("TopNav", () => {
  it("renders the logo, nav items and right slot", () => {
    render(
      <MemoryRouter initialEntries={["/agent/dashboard"]}>
        <TopNav logo={<span>Swift</span>} items={items} right={<button>Sign out</button>} />
      </MemoryRouter>,
    );
  ...
  it("marks the current route's link active", () => {
    ...
    expect(screen.getByRole("link", { name: "Tickets" })).toHaveClass("text-primary-text");
    expect(screen.getByRole("link", { name: "Dashboard" })).toHaveClass("text-text-secondary");
  });
});
